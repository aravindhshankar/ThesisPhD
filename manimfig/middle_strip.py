...
class MiddleStrip(Scene):
	def construct(self):
		textcol = YELLOW
		lefttext = Text('Strongly correlated electrons in Sachdev-Ye-Kitaev models and Twisted bilayer graphene', color = textcol, disable_ligatures = True, font_size=30).scale(1).align_on_border([-1,0,0], buff=1.2)
		righttext = Text('A. S. Shankar', color = textcol, disable_ligatures = True, font_size=30).scale(1).align_on_border([1,0,0], buff=1.2)
